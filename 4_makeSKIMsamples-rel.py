...
def make_relative_path(absolute_path, base_path):
...
#convert absolute paths in fwd and rev read path columns to relative paths
    base_path = os.getcwd()
    merged_df['forward'] = merged_df['forward'].apply(lambda x: make_relative_path(x, base_path))
    merged_df['reverse'] = merged_df['reverse'].apply(lambda x: make_relative_path(x, base_path))
